...
public class DatapusherRequester
{
    ...
    {
        if (m_useApiAuth)
            return await SendRequestAsyncUserAPIQuery(path, method, content, mediaResponseType);
        else
            return await SendRequestAsyncUseAccount(path, method, content, mediaResponseType);
    }

    private async Task<HttpResponseMessage> SendRequestAsyncUserAPIQuery(string path, HttpMethod method, HttpContent content, string mediaResponseType)
    {
        APIQuery query = new APIQuery(m_Key, m_Token, m_baseUrl.Split(';'));

        void ConfigureRequestAPIRequest(HttpRequestMessage request)
        {
            if (!(content is null))
        ...
        return await query.SendWebRequestAsync(ConfigureRequestAPIRequest, path);
    }

    private async Task<HttpResponseMessage> SendRequestAsyncUseAccount(string path, HttpMethod method, HttpContent content, string mediaResponseType)
    {
