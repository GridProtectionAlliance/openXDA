...
namespace openXDA.Controllers
{
    ...
    protected virtual string Connection { get; } = "systemSettings";
        ...
        */
        ...
        /// <summary>
        /// Runs set of historic Data through the Alarm to evaluate Performance
        /// </summary>
        /// <returns>A List of Errors if applicaple</returns>
        /*[HttpPost, Route("SaveGroup")]
        public IHttpActionResult SaveAlarmGroup([FromBody] TestGroup data)
        {
            try
            {
                // Figure out Channels
                ...
                if (data.generalSettings.MeterIDs.Count == 0 || data.generalSettings.BaseVoltages.Count == 0 || data.generalSettings.PhaseIDs.Count == 0)
                    return InternalServerError();

                string condition = $@"MeterID in ({string.Join(",", data.generalSettings.MeterIDs)}) AND 
                                    PhaseID in ({string.Join(",", data.generalSettings.PhaseIDs)}) AND
                                    (SELECT VoltageKV FROM ASSET WHERE Asset.ID = ChannelDetail.AssetID) in ({string.Join(",", data.generalSettings.BaseVoltages)}) AND
                                    (
                                        SELECT ID 
                                            FROM ChannelGroupType
                                            WHERE
                                                ChannelGroupType.MeasurementTypeID = ChannelDetail.MeasurementTypeID AND
                                                ChannelGroupType.MeasurementCharacteristicID = ChannelDetail.MeasurementCharacteristicID
                                    ) = {data.generalSettings.MeasurementTypeID} AND
                                    SeriesType = 'Average'
                                    ";
                ...
                using (AdoDataConnection connection = new AdoDataConnection(Connection))
                {
                    ...
                    channels = channelTbl.QueryRecordsWhere(condition).ToList();

                    //Create Alarm group
                    AlarmGroup group = new AlarmGroup()
                    {
                        ID = data.generalSettings.ID,
                        Name = data.generalSettings.Name,
                        Formula = data.setpointEvaluation.setPoint,
                        AlarmTypeID = data.generalSettings.AlarmTypeID,
                    };
                    ...
                    Token root = new Token(data.setpointEvaluation.setPoint);
                    double value = root.ComputeScalar(data.setpointEvaluation.channelIds, data.setpointEvaluation.startTime, 1000);

                    SeriesType seriesType = new TableOperations<SeriesType>(connection).QueryRecordWhere("Name = {0}", data.generalSettings.IntervallDataType);
                    channels.ForEach(ch =>
                    {

                        Series series = seriesTbl.QueryRecordWhere("ChannelID = {0} AND SeriesID = {1}", ch.ID, seriesType.ID);
                        Alarm alarm = alarmTbl.QueryRecordWhere("AlarmGroupID = {0} AND SeriesID = {1}", group.ID, series.ID);
                        if (alarm == null)
                        {
                            alarmTbl.AddNewOrUpdateRecord(new Alarm() { AlarmGroupID = group.ID, SeriesID = ch.ID });
                            alarm = alarmTbl.QueryRecordWhere("AlarmGroupID = {0} AND SeriesID = {1}", group.ID, series.ID);
                        }

                        // Add Factors
                        data.factorList.ForEach(factor => {
                            factorTbl.AddNewRecord(new AlarmFactor() { AlarmID = alarm.ID, SeverityID = factor.SeverityID, Factor = factor.Value });
                        });

                        // Add Values
                        valueTbl.AddNewRecord(new AlarmValue() { AlarmID = alarm.ID, AlarmdayID = null, EndHour = null, StartHour = 0, Value = value });
                    });
