...
import { updateStatistics, selectStatisticSettings, selectChannelRequest, updateStatChannels, updateAllChannels } from './StaticWizzardSlice';
...
const SelectHistoricalData = (props: IProps) => {
    const dispatch = useDispatch();


    const statisticSettings = useSelector(selectStatisticSettings);
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            ...
            <div className="row" style={{ margin: 0 }}>
                <div className="col-6">
                    <DateRangePicker<SPCTools.IStatisticData> Label={''} Record={statisticSettings} FromField={'StartDate'} ToField={'EndDate'} Setter={(r) => dispatch(updateStatistics(r))} />
                </div>
            </div>
            <div className="row" style={{ margin: 0 }}>
                <div className="col">
            ...
            <div className="row" style={{ margin: 0 }}>
                ...
                <div className="col-4">
                    <DataFilter filter={statisticSettings.DataFilter} setter={(r) => dispatch(updateStatistics({ ...statisticSettings, DataFilter: r }))} />
                </div>
            </div>
        </div>      
    );
...
const ChannelTable = (props: { selectAll: boolean }) => {
    const channelRequest = useSelector(selectChannelRequest)
    const dispatch = useDispatch();

    const [allChannels, setAllChannels] = React.useState<Array<openXDA.IChannel>>([]);
    const [sort, setSort] = React.useState<keyof openXDA.IChannel>('MeterName');
    const [asc, setAsc] = React.useState<boolean>(false);

    React.useEffect(() => {
        setAllChannels((old) => { return _.orderBy(old, [sort], [asc ? "asc" : "desc"]) })
    }, [sort, asc])

    React.useEffect(() => {
        let handle = getChannels();
    ...
    function getChannels(): JQuery.jqXHR<string> {
        let handle = $.ajax({
            type: "POST",
            url: `${apiHomePath}api/SPCTools/StaticAlarmCreation/Channel`,
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
        ...
        handle.done(data => { setAllChannels(data); dispatch(updateAllChannels(data)) })
        return handle;
    }
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            <MultiSelectTable<openXDA.IChannel>
                ...
                selected={(item) => false}
                primaryKey={'ID'}
                updateSelection={(selected) => { dispatch(updateStatChannels(selected));}}
                selectAll={props.selectAll}
            />
        </div>
    )
