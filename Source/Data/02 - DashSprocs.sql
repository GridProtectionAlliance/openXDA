...
BEGIN
	...
	declare @FaultSummaryIDs TABLE (ID int);
	declare @TheEventDate as Date = CAST(@EventDate as Date)

	insert into @EventIDs ( ID ) (
	Select distinct [dbo].[FaultSummary].[EventID] FROM [dbo].[FaultSummary] 
	join Event on Event.ID = FaultSummary.EventID
	join Meter on Meter.ID = Event.MeterID 
	where CAST([Inception] as Date) = CAST(@EventDate as Date) and
	MeterID in ( Select * from @MeterIDs)
	)

	DECLARE @eventid as int

	DECLARE site_cursor CURSOR FOR Select ID from @EventIDs

	OPEN site_cursor

	FETCH NEXT FROM site_cursor into @eventid
	...
	--print @eventid
		...
		insert into @FaultSummaryIDs ( ID )

			Select top 1 [dbo].[FaultSummary].[ID] FROM [dbo].[FaultSummary] where 
			CAST([Inception] as Date) = @TheEventDate 
			and [dbo].[FaultSummary].[EventID] = @eventid
			and [dbo].[FaultSummary].[IsSuppressed] = 0
			--and [dbo].[FaultSummary].[IsValid] = 1
			and [dbo].[FaultSummary].[IsSelectedAlgorithm] = 1
			order by [dbo].[FaultSummary].[Inception] ASC
	...
	--declare  @FaultsTable TABLE (MeterID int, LineID int, EventID int, InceptionTime DateTime, FaultType nvarchar(10), FaultDuration DateTime, Distance float, Method nvarchar(20));

	SELECT
		[dbo].[Meter].[Name] as thesite,
		[dbo].[Meter].[ShortName] as theshortsite,
		[dbo].[MeterLocation].[ShortName] as locationname,
		[dbo].[Event].[MeterID] as themeterid,
		[dbo].[Event].[LineID] as thelineid,
		[EventID] as theeventid,
		[dbo].[MeterLine].[LineName] as thelinename,
		[dbo].[Line].[VoltageKV] as voltage,
		Cast(Cast([dbo].[Event].[StartTime] as Time) as nvarchar(100)) as theinceptiontime,
		[FaultType] as thefaulttype,
		--[DurationSeconds] as theduration,
		CAST([Distance] as decimal(16,2)) as thecurrentdistance

		FROM [dbo].[FaultSummary] join 
		[dbo].[Event] on [dbo].[Event].[ID] = [dbo].[FaultSummary].[EventID] join 
		[dbo].[Meter] on [dbo].[Event].[MeterID] = [dbo].[Meter].[ID] join
		[dbo].[MeterLocation] on [dbo].[Meter].[MeterLocationID] = [dbo].[MeterLocation].[ID] join
		[dbo].[Line] on [dbo].[Event].[LineID] = [dbo].[Line].[ID] join
		[dbo].[MeterLine] on [dbo].[Event].[LineID] = [dbo].[MeterLine].[LineID] and [dbo].[MeterLine].[MeterID] = [dbo].[Event].[MeterID]
		where
		[dbo].[FaultSummary].[ID] in (Select ID from @FaultSummaryIDs)
		order by [Inception]
